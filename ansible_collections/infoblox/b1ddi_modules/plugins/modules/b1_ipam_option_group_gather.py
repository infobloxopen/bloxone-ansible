...
short_description: Configure Option Group on Infoblox BloxOne DDI
version_added: "1.0.1"
description:
  - Gather facts about Option Groups in Infoblox BloxOne DDI. This module manages the gather fact of IPAM Option Group object using BloxOne REST APIs.
requirements:
  - requests
options:
  api_key:
...
EXAMPLES = '''
    ...
    - name: Gather Option Group
      b1_ipam_option_group_gather:
        host: "{{ host }}"
        api_key: "{{ api }}"
        state: gather
      register: option_groups
    - debug: msg="{{ option_groups }}"
'''
...
def get_option_group(data):
    ...
    endpoint = f'/api/ddi/v1/dhcp/option_group'
