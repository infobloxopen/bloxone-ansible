...
try:
    ...
    from ipam import IpSpaceApi, IPSpace
except ImportError:
...
class IpSpaceModule(BloxoneAnsibleModule):
    ...
    def payload_params(self):
        ...
        # remove none values or if key is not in __props__ of IPSpace
    ...
    def payload(self):
        ...
        self._payload = IPSpace.from_dict(self.payload_params)
        return self._payload
    ...
    def find(self):
        if self.params['id'] is not None:
            ...
            filter = "name=='{0}'".format(self.params['name'])
            resp = IpSpaceApi(self.client).list(filter=filter, inherit="full")
            if len(resp.results) == 1:
                return resp.results[0]
            if len(resp.results) > 1:
                self.fail_json(msg="Found multiple IpSpace: {0}".format(resp.results))
