...
try:
    ...
    from ipam import SubnetApi, Subnet
except ImportError:
...
class SubnetModule(BloxoneAnsibleModule):
    ...
    def payload_params(self):
        ...
        # remove none values or if key is not in __props__ of Subnet
    ...
    def payload(self):
        ...
        self._payload = Subnet.from_dict(self.payload_params)
        return self._payload
    ...
    def find(self):
        if self.params['id'] is not None:
            ...
            filter = "address=='{0}' and space=='{1}'".format(self.params['address'], self.params['space'])
            resp = SubnetApi(self.client).list(filter=filter, inherit="full")
            if len(resp.results) == 1:
                return resp.results[0]
            if len(resp.results) > 1:
                self.fail_json(msg="Found multiple Subnet: {0}".format(resp.results))
